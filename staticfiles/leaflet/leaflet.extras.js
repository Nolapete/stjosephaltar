...
L.Map.DjangoMap = L.Map.extend({
  initialize: function (id, options) {
    ...
    options = L.Util.extend(options, this._projectionOptions(djoptions));
    if (djoptions.extent) {
  ...
  _projectionOptions: function (djoptions) {
    ...
    function computeMaxResolution(bbox) {
      ...
      var aspect = Math.floor(
        Math.max(width, height) / Math.min(width, height) + 0.5,
      );
  ...
  _djSetupControls: function () {
    ...
    if (
      ...
      this.attributionControl.setPrefix(
        this.options.djoptions.attributionprefix,
      );
    }
...
L.Map.djangoMap = function (id, options) {
  ...
  if (options.callback === null) {
    ...
    if (typeof defaultcb == "function") {
      ...
      if (console)
        console.warn(
          "DEPRECATED: Use of default callback " +
            defaultcb.name +
            "() is deprecated (see documentation).",
        );
    }
  }
