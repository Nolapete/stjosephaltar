L.FieldStore = L.Class.extend({
  ...
  _serialize: function (layer) {
    var items =
        typeof layer.getLayers == "function" ? layer.getLayers() : [layer],
    ...
    var is_geometrycollection =
      geojson.geometry && geojson.geometry.type == "GeometryCollection";
...
L.GeometryField = L.Class.extend({
  ...
  initialize: function (options) {
    ...
    window.onbeforeunload = L.Util.bind(function (e) {
      if (this._unsavedChanges) return L.GeometryField.unsavedText;
      if (typeof _beforeunload == "function") return _beforeunload();
    }, this);
  },
  ...
  addTo: function (map) {
    ...
    this.store = new this.options.field_store_class(
    ...
    var drawControl = (this._drawControl = new L.Control.Draw(
      this._controlDrawOptions(),
    ));
    ...
    if (this.options.modifiable) {
      ...
      for (var toolbar in drawControl._toolbars) {
        drawControl._toolbars[toolbar].on(
          "enable disable",
          function (e) {
      ...
      map.on(
        ...
        function (e) {
          ...
          if (!this._acceptDrawEvents) return;
  ...
  load: function () {
    ...
    if (geometry) {
      ...
      if (geometry instanceof L.LayerGroup) {
        ...
        this.options.collection_type !== "featureGroup" &&
